...
def prueba():
    ...
    imagen = _abrir_archivo("images/img23.jpg")
    ...
    for c in cnts:
        ...
        print("contornos de corte:", approx)
    ...
    thresh2 = cv2.threshold(adjust, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
...
#///// IMPORTANTE USAR LA LINEA DEBAJO PARA COLOCAR CON QUE TRATAR LA Imagen
#//// SEGUN SEA EL CONTRASTE DE LA MISMA MIENTRAS SE CONFIGURA ALGO MEJOR
    cnts = cv2.findContours(thresh2.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    print("encuentra contornos: ", len(cnts))
    cnts = cnts[0] if imutils.is_cv2() else cnts[1]
    print("encuentra contornos imutils.is_cv2: ", len(cnts))
    digitCnts = []
    ...
    try:
        ...
        # display the digits
        print("{}{}.{} \x52\x50\x4D".format(*digits))

        #print("{}{}.{} \u03bcA".format(*digits)) # Se retira de esta línea lo siguiente \u00b0C, esto es °class
        #Tambien al quitar la u del inicio mantiene el resultado
        # Linea original print(u"{}{}.{} \u00b0C".format(*digits))
    ...
    #cv2.imshow("ventana1", thresh)
    cv2.imshow("ventana2", thresh2)
    #cv2.imshow("ventana3", thresh3)
